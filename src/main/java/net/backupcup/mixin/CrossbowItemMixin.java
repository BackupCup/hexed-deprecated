...
public abstract class CrossbowItemMixin {
    ...
    @Redirect(method = "loadProjectile", at = @At(value = "INVOKE", target = "Lnet/minecraft/item/CrossbowItem;putProjectile(Lnet/minecraft/item/ItemStack;Lnet/minecraft/item/ItemStack;)V"))
    private static void hexed$CelebrationPutFirework(ItemStack crossbow, ItemStack projectile) {
    ...
    private static void hexed$CelebrationAccuracy(Args args) {
        ...
        if (HexHelper.INSTANCE.hasEnchantmentInSlot(crossbow, RegisterEnchantments.INSTANCE.getCELEBRATION_HEX())) {
            ...
            args.set(7, Random.Default.nextDouble(0, 1) <= 0.333 ? 0.0625f : speed + (float) Random.Default.nextDouble(-0.75, 0));
            args.set(9, divergence + (float) Random.Default.nextDouble(-5, 5));
        }
    }
}
